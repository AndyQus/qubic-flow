...
<template>
  <header class="border-b border-qubic-border bg-qubic-card/40 backdrop-blur-glass">
    <div class="w-full px-4 sm:px-6 lg:px-8 py-3 flex items-start justify-between">
      <router-link to="/">
        <img :src="logoUrl" alt="QubicFlow" class="h-[72px] w-auto" />
      </router-link>

      <!-- Right side: controls top, thank-you text below -->
      <div class="flex flex-col items-end gap-1 pt-1 min-w-max">

        <!-- Controls row: mobile = eye top / pill below; desktop = side by side -->
        <div class="flex flex-col-reverse sm:flex-row items-end sm:items-center gap-2 sm:gap-3">
        ...
        <!-- Dankeschön-Text (nur Desktop) -->
        <router-link v-if="isSuppressed" to="/support"
          class="hidden sm:flex items-center gap-1.5 text-xs font-semibold transition-colors"
          :class="isLight ? 'text-emerald-600 hover:text-emerald-800' : 'text-[#9acd32] hover:text-[#b8e05a]'">
